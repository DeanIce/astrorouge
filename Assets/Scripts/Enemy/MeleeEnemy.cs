...
public class MeleeEnemy : MonoBehaviour, IEnemy
{
    ...
    public void Hunt(Collider target)
    {
        ...
        //attacking
        if (Mathf.Abs(targetRb.transform.position.x - transform.position.x) < attackRange && Mathf.Abs(targetRb.transform.position.y - transform.position.y) < attackRange && Mathf.Abs(targetRb.transform.position.z - transform.position.z) < attackRange && attackers < 2)
        {
            if (!attacking)
            {
                attacking = true;
